...
const CONFIG_DIR_PATH = './config';
const FEEDS_FILE_PATH = `${CONFIG_DIR_PATH}/feeds.json`;
const EXTRA_CONFIG_FILE_PATH = `${CONFIG_DIR_PATH}/extraConfig.json`;
const GLOBAL_CONFIG_FILE_PATH = `${CONFIG_DIR_PATH}/globalConfig.json`;

const TEMPLATES_DIR_PATH = './templates';
const TEMPLATE_BUS_PATH = `${TEMPLATES_DIR_PATH}/bus.svg`;
const TEMPLATE_CIRCLE_PATH = `${TEMPLATES_DIR_PATH}/circle.svg`;
const TEMPLATE_ARROW_PATH = `${TEMPLATES_DIR_PATH}/arrow.svg`;


const OUTPUT_DIR_PATH = '../output';
const OUTPUT_ALL_ICONS_PATH = `${OUTPUT_DIR_PATH}/allIcons.json`;
const OUTPUT_ALL_SHAPES_PATH = `${OUTPUT_DIR_PATH}/allShapes.json`;
const OUTPUT_COLORS_LIST_PATH = `${OUTPUT_DIR_PATH}/colorsList.json`;
const OUTPUT_DATA_DIR_PATH = `${OUTPUT_DIR_PATH}/data`;


const feeds = JSON.parse(fs.readFileSync(FEEDS_FILE_PATH, 'utf8')).all;
const extraConfig = JSON.parse(fs.readFileSync(EXTRA_CONFIG_FILE_PATH, 'utf8'));
const globalConfig = JSON.parse(fs.readFileSync(GLOBAL_CONFIG_FILE_PATH, 'utf8'));
...
const updateFeed = async (feed) => {
  if (!fs.existsSync(`${OUTPUT_DATA_DIR_PATH}/${feed.username}`)) {
    fs.mkdirSync(`${OUTPUT_DATA_DIR_PATH}/${feed.username}`);
    fs.mkdirSync(`${OUTPUT_DATA_DIR_PATH}/${feed.username}/icons`);
  }
  ...
  */
  ...
  const busTemplate = fs.readFileSync(TEMPLATE_BUS_PATH, 'utf8');
  const circleTemplate = fs.readFileSync(TEMPLATE_CIRCLE_PATH, 'utf8');
  const arrowTemplate = fs.readFileSync(TEMPLATE_ARROW_PATH, 'utf8');
  ...
  routes.map((route) => {
    ...
    sharp(busBuffer)
      .resize(64, 64)
      .png()
      .toFile(`${OUTPUT_DATA_DIR_PATH}/${feed.username}/icons/${actualColor.toUpperCase()}_bus.png`, (err, info) => {
        if (err) throw err;
    ...
    sharp(arrowBuffer)
      .resize(120, 120)
      .png()
      .toFile(`${OUTPUT_DATA_DIR_PATH}/${feed.username}/icons/${actualColor.toUpperCase()}_arrow.png`, (err, info) => {
        if (err) throw err;
        //console.log(`${routeColor[0]}_bus.png generated for ${feed.fullname}`)
      });
    ...
    sharp(circleBuffer)
      .resize(64, 64)
      .png()
      .toFile(`${OUTPUT_DATA_DIR_PATH}/${feed.username}/icons/${actualColor.toUpperCase()}_circle.png`, (err, info) => {
        if (err) throw err;
        //console.log(`${routeColor[0]}_bus.png generated for ${feed.fullname}`)
      });
  ...
  fs.writeFileSync(`${OUTPUT_DATA_DIR_PATH}/${feed.username}/icons.json`, JSON.stringify(iconsRef));
  ...
  fs.writeFileSync(`${OUTPUT_DATA_DIR_PATH}/${feed.username}/shapes.json`, JSON.stringify({
...
const updateFeeds = async () => {
  const onlyThese = ['harvard'];
  const filteredFeeds = feeds.filter((feed) => onlyThese.includes(feed.username));
  const augmentedFilteredFeeds = filteredFeeds.map((feed) => {
  ...
  for (const feed of augmentedFilteredFeeds) {
    console.log(feed);
    await updateFeed(feed);
  }
  fs.writeFileSync(OUTPUT_ALL_ICONS_PATH, JSON.stringify(imagesList));
  fs.writeFileSync(OUTPUT_ALL_SHAPES_PATH, JSON.stringify(shapesList));
  fs.writeFileSync(OUTPUT_COLORS_LIST_PATH, JSON.stringify([...new Set(colorsList.sort())]));
};

if (fs.existsSync(OUTPUT_DATA_DIR_PATH)) {
  fs.rmSync(OUTPUT_DATA_DIR_PATH, { recursive: true });
}

fs.mkdirSync(OUTPUT_DATA_DIR_PATH, { recursive: true });
